...
export default function HumidorScreen() {
  ...
  const processImageWithAI = async () => {
      ...
      startLoadingMessages(); // Keep this

      try {
          const userId = auth.currentUser?.uid || 'test-user';
          const aiAnalysisString = await analyzeCigarImage(image, userId); // This will now always be a JSON string

          // Stop loading messages as AI call is done
          if (loadingInterval) {
              clearInterval(loadingInterval);
          ...
          if (aiAnalysisString) {
              setAiResponse(aiAnalysisString); // Store the raw JSON string
              let parsedData;
              try {
                  parsedData = JSON.parse(aiAnalysisString);
              } catch (e) {
                  console.error('Failed to parse supposedly valid JSON from AI:', e, aiAnalysisString);
                  Alert.alert('AI Response Error', 'The AI response was not in the expected format. Please try again.');
                  setProcessingModal(false);
                  setModalVisible(true); // Go back to allow retry
                  return;
              }
              
              // Local best guess to improve offline UX or early-stage UI
              const bestMatch = searchCigars(parsedData.fullName || parsedData.cigarBrand || '');

              if (bestMatch) {
                parsedData.cigarBrand = bestMatch.brand;
                parsedData.cigarLine = bestMatch.line;
              }

              // Use a default name if AI doesn't provide one, to ensure "Save" can be enabled.
              // The AI response should now always have `fullName`.
              const cigarNameFromAI = parsedData.fullName || parsedData.cigarBrand || 'New Cigar Entry';

              setNewCigar({
                  ...newCigar, // Keep existing notes and rating if user set them
                  cigarName: cigarNameFromAI,
                  // Do NOT reset notes/rating here unless intended
              });
              
              // Reset feedback states for new analysis
              ...
              // This case should be less likely if openai.js always returns a string
              console.error('AI analysis returned null/empty string unexpectedly');
              if (loadingInterval) clearInterval(loadingInterval);
              setProcessingModal(false);
              setModalVisible(true);
              Alert.alert('Error', 'Failed to analyze image. No response from AI. Please try again.');
          }
      } catch (error) {
  ...
  const uploadCigar = async () => {
      ...
      try {
          ...
          const logEntryData = {
              ...
              aiRawResponseSnapshot: aiResponse || ""
          };
          ...
          try {
              ...
              try { if (aiResponse) offlineParsedAiData = JSON.parse(aiResponse); } catch { /* ignore */ }
                                  ...
                                  <Text style={styles.aiAnalysisText}>
                                      {(() => {
                                          ...
                                          try {
                                              ...
                                              return aiResponse.substring(0, 300) + (aiResponse.length > 300 ? '...' : '');
                                          }
                                      })()}
                                  </Text>
                              </View>
